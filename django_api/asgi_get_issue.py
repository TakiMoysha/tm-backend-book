...
def test_user_controller_with_msgspec(dmr_client: DMRClient, dmr_rf: DMRRequestFactory) -> None:
    ...
    all_users = UserController.as_view()(dmr_rf.get("/api/users/"), data=request_data)
...
serializer = SimpleNamespace(serialize=lambda obj: msgspec.msgpack.encode(obj))
...
def method_get(s: ISerializer):
    s.serialize(["hello world"])


method_get(cast(ISerializer, serializer))
